...
public class ChromeDriverUpdater
{
    public async Task UpdateChromeDriverAsync()
    {
        ...
        string windowsDownloadUrl = $"https://storage.googleapis.com/chrome-for-testing-public/{latestVersion}/win64/chromedriver-win64.zip";
       // string macDownloadUrl = $"https://edgedl.me.gvt1.com/edgedl/chrome/chrome-for-testing/{latestVersion}/mac-x64/chromedriver-mac-x64.zip";
        string linuxDownloadUrl = $"https://storage.googleapis.com/chrome-for-testing-public/{latestVersion}/linux64/chromedriver-linux64.zip";
       ...
       // UpdateChromeDriver(macDownloadUrl, Path.Combine(chromeDriverFolder, "chromedriver"), "chromedriver");
        UpdateChromeDriver(linuxDownloadUrl, Path.Combine(chromeDriverFolder, "chromedriver"), "chrome");
        ...
        using (HttpClient client = new HttpClient())
        {
          //  try {
            ...
            using (var archive = new ZipArchive(new MemoryStream(driverBytes), ZipArchiveMode.Read))
            {
                ...
                // Get all processes locking the target file
                Process[] processes = Process.GetProcessesByName(Path.GetFileNameWithoutExtension(targetPath));
